...
import { 
  ...
  Lightbulb, Search, MoreHorizontal, User, ClipboardCopy, FileText, ImageIcon, X, UploadCloud
} from 'lucide-react';
...
export default function SubmissionsAdminDashboard() {
  ...
  const handleOpenDialog = (form?: FormRecord) => {
    if (form) {
      ...
      setFormData({ title: '', description: '', embedUrl: '', sheetApiUrl: '', bannerUrl: '', status: 'active' });
    }
  ...
  return (
    <div className="container mx-auto py-8 space-y-8">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <Button onClick={() => handleOpenDialog()} className="h-12 rounded-xl shadow-lg">
          <PlusCircle className="mr-2 h-5 w-5" /> Connect Google Form
        </Button>
      </div>
            ...
            <CardContent>
              <Table>
                ...
                <TableBody>
                  ...
                  {forms.length === 0 && (
                    <TableRow><TableCell colSpan={3} className="text-center py-20 text-muted-foreground">No integrated forms found.</TableCell></TableRow>
                  )}
                </TableBody>
              </Table>
            </CardContent>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="sm:max-w-lg rounded-2xl">
          <DialogHeader>
            <DialogTitle>{editingFormId ? 'Edit Connection' : 'Connect Google Form'}</DialogTitle>
            <DialogDescription>Link an existing Google Form to the portal.</DialogDescription>
          </DialogHeader>
          <form onSubmit={handleSaveForm} className="space-y-4 py-4">
            ...
            <DialogFooter className="pt-4 gap-2">
              ...
              <Button type="submit" disabled={isProcessing} className="h-11 rounded-xl flex-1 font-bold">
                {isProcessing ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : (editingFormId ? 'Update' : 'Connect')}
              </Button>
            </DialogFooter>
          </form>
        </DialogContent>
